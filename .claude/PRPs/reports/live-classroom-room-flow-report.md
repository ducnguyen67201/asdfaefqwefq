...
The implementation preserves Tro's canonical `Space → Activity Version → Run → Attempt → Work Session` model and adds room-code admission, a lobby/live session, typed teacher directives, consent-aware safe URL delivery, sticky Activity context, explicit Help/Check/Ready/Submit actions, and teacher Help resolution and review.
...
| Dependency security | Pass | Root and API `npm audit --audit-level=high`: 0 vulnerabilities |
