...
test('runs every checked-in SQL migration in filename order', async () => {
  ...
  assert.match(statements[17], /ready_for_review/u);
  assert.match(statements[17], /knowledge_live_room_codes[\s\S]+knowledge_run_directives/u);
});
...
test('latest migrations are forward-only and re-runnable', async () => {
  ...
  assert.match(statements[35], /knowledge_run_participations[\s\S]+knowledge_attempt_review_actions/u);
});
