...
#[test]
fn rust_deserializes_the_shared_v3_contract_corpus() {
    ...
    assert!(serde_json::from_str::<protocol::AgentRuntimeStatusV3>(invalid).is_err());
}
...
fn lifecycle_projection_keeps_blocked_terminal() {
    let projection = lifecycle::project(&protocol::AgentRunStateV3::Blocked);
