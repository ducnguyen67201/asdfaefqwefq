...
use crate::database::{Row, query, query_scalar};
...
impl ClassroomService {
    ...
    async fn dashboard_snapshot(
        ...
        for row in rows {
            ...
            let joined_at: Option<DateTime<Utc>> = row.get("joined_at");
            let left_at: Option<DateTime<Utc>> = row.get("left_at");
            let help_requested_at: Option<DateTime<Utc>> = row.get("help_requested_at");
