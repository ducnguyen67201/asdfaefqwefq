...
use crate::database::{PgRow, Row, query, query_scalar};
...
impl ClassroomService {
    ...
    pub async fn join_room(
        ...
        let revoked_at: Option<chrono::DateTime<Utc>> = room.get("revoked_at");
        let expires_at: chrono::DateTime<Utc> = room.get("expires_at");
    ...
    pub async fn leave_session(
        ...
        if let Some(left_at) = current.get::<Option<chrono::DateTime<Utc>>, _>("left_at") {
            transaction
                .commit()
                .await
                .map_err(|error| ApiError::database(&error))?;
        ...
        let left_at: chrono::DateTime<Utc> = query_scalar(
