...
function imageFor(buffer: Buffer, width?: number, height?: number) {
  ...
  return {
    ...
    toPNG: () =>
      marker === 1 || marker === 2 ? NORMALIZED_PNG : marker === 3 ? GENERATED_PNG : ACTIVE_PNG,
  };
}
...
describe('CompanionCustomizationService', () => {
  ...
  function harness(overrides: {
    ...
    const fetchImpl: typeof fetch =
      ...
      (async (input, init) => {
        ...
        return new Response(
          JSON.stringify({
            imageBase64: GENERATED_PNG.toString('base64'),
            mimeType: 'image/png',
  ...
  it('normalizes one source, keeps a memory preview, then encrypts activation', async () => {
    ...
    expect(encryptedFiles).toHaveLength(1);
    const stored = await readFile(
      path.join(
        root,
        'companion-customizations',
        ownerDirectories[0] as string,
        encryptedFiles[0] as string,
      ),
      'utf8',
    );
    ...
    expect((await service.handleRequest(new Request(activeUrl))).status).toBe(404);
  });
  ...
  it('recovers an older encrypted asset and rewrites it for key rotation', async () => {
    ...
    expect(files[0]).not.toContain('f'.repeat(64));
  });
